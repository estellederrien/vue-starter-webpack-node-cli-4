...
*/
module.exports = function(app, db, middleware, ftpConfig, nodeFilePath, fs, bodyParser) {
    ...
    var storageFiles = multer.diskStorage({
        destination: function(req, file, cb) {
            cb(null, nodeFilePath); // nodeFilePath to store our files . Le chemin ou on enregistre nos fichiers.
        },
    ...
    app.post("/deleteFile", middleware.permission_valid("DELETE_FILE"), function(req, res) {
        ...
        try {
            var fs = require("fs");
            var filenodeFilePath = nodeFilePath + file;
            fs.unlinkSync(filenodeFilePath);
            res.sendStatus(200);
        } catch (e) {
            console.log(e);
            res.sendStatus(400);
     ...
     * @error   NONE
     */
    app.post("/createFtpFiles", uploadFiles.array("file", 10), middleware.requiresLoggedIn, middleware.permission_valid("CREATE_FILE"), function(
        req,
        res,
        next
    ) {
...
/* TODO SFTP SERVER PARAMS 'STORING FILES AND PICTURES ON MY PERSONAL FTP' */
/*     let sftp = new Client()
    sftp.connect(config)
        .then(() => {
